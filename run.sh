...
if [ "$PROXY_MODE" = "https" ]; then
  PORT_ARGS="-p 8080:8080 -p 8443:8443 -p 9101:9100"
...
docker run -d --name $CONTAINER_NAME --network domino-net --ulimit nofile=65536:65536 -v "$(pwd)/cfg:/cfg" $PORT_ARGS $NGINX_CONF_ARGS $MODE_ARGS $ACME_ARGS -e NGINX_LOG_LEVEL=$NGINX_LOG_LEVEL --hostname $CONTAINER_HOSTNAME $CONTAINER_IMAGE:$IMAGE_TAG
